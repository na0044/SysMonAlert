...
class TestSystemMetrics(unittest.TestCase):
    ...
    def test_log_metrics(
        ...
        mock_virtual_memory.return_value = type(
            "vmem", (object,), {"percent": 91}
        )()  # Above memory threshold
        mock_disk_usage.return_value = type(
            "disk", (object,), {"percent": 92}
        )()  # Above disk threshold
        mock_net_io.return_value = type(
            "netio", (object,), {"bytes_sent": 1000, "bytes_recv": 2000}
        )()

        # Run the function for one iteration
        with patch("time.strftime", return_value="2025-01-17 12:00:00"):
            with self.assertRaises(StopIteration):  # Break loop after first iteration
                log_metrics()
